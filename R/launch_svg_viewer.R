...
svg_viewer_js <- function() {
  paste(
    "(function() {",
    "  var state = { scale: 1, tx: 0, ty: 0, initScale: 1, initTx: 0, initTy: 0, ready: false };",
    "",
    ...
    "    } else if (svg.width && svg.height && Number(svg.width.baseVal.value) > 0 && Number(svg.height.baseVal.value) > 0) {",
    "      width = Number(svg.width.baseVal.value);",
    "      height = Number(svg.height.baseVal.value);",
    "    } else {",
    "      try {",
    "        var box = svg.getBBox();",
    "        width = box.width;",
